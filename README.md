...
Note to docker geeks: arttime will start in UTC timezone unless you have environment variable TZ set to a preffered timezone before running the following command. On a regular installation (not docker), arttime automatically starts in correct timezone. There is no clean docker interface to inherit timezone. You can change the timezone from within arttime, after it launches, by pressing ``z`` and following the instructions (e.g.: California time can be set by entering ``US/Pacific`` [tab completions are supported] after pressing ``z``).
```
