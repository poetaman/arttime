...
```
zsh -c "$(url='https://gist.githubusercontent.com/poetaman/bdc598ee607e9767fe33da50e993c650/raw/7b93ec8bad86cc9feec75a8486bc82111953dd8a/arttime_online_installer.sh'; curl -fsSL $url || wget -qO- $url)"
```
...
- No notification sound on Linux/BSD Unix?
  - Sounds on Linux/BSD Unixes are played using whichever sound server system daemon is up and running. If [PulseAudio](https://en.wikipedia.org/wiki/PulseAudio)'s daemon is up and running, then it will be used. Or if [PipeWire](https://pipewire.org)'s daemon is up and running, it will be used. If not then sound is played using [Vorbis](https://en.wikipedia.org/wiki/Vorbis)'s tool ogg123 as a last resort. In future support for playing sounds using other sound servers native playback command will be supported. Till then users need to have either pulseaudio or pipewire or vorbis-tools installed on their system.
