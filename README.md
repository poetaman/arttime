...
- Expressive timers: Associate different art, color, and message with timers you would frequently use. This makes your timers more expressive, and personalized. To check preinstalled examples: press ``k``, type ``timer`` and press ``Tab``.
- Storyboards: Quickly create art storyboards by sequencing art and messages from arttime's collection. To check preinstalled examples: press ``k``, type ``story`` (or ``learn``) and press ``Tab``.
