...
- Sleep friendly: start arttime or reset start-time (press ``r``) before putting computer to sleep. Find the elapsed time between sleep and wakeup. No, it doesn't consume power when computer is sleeping!
...
3. Animated art clock. ``$ arttime --nolearn -a military -b military2 -t "Say no to wars, yes to peace" --ac 4``. Note: animation works only if a-art and b-art height is same, which it is for many related arts.
