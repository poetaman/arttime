...
- No notification sound on Linux/BSD Unix?
  - Sounds on Linux/BSD Unixes are played using whichever sound server system daemon is up and running. If [PulseAudio](https://en.wikipedia.org/wiki/PulseAudio)'s daemon is up and running, then it will be used. If not then sound is played using [Vorbis](https://en.wikipedia.org/wiki/Vorbis)'s tool ogg123 is used. In future support for playing sounds using other sound servers native playback command will be supported. Till then users need to have either pulseaudio or vorbis-tools installed on their system.
